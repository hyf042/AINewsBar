...
macOS 菜单栏多分类资讯阅读器。3 个 tab（**AI / 财报 / 新闻**），26 个内置精选 RSS 源，通过阿里云百炼（默认 qwen3.6-plus）生成中文摘要 + AI 推荐 + 每日概述；财报 tab 启用 AI Filter 过滤非财报噪声。
...
### RSS 抓取
- **26 个内置精选源**（11 AI + 8 财报 + 7 新闻；中英混合）
...
- **AI 推荐 5 篇** — 基于标题+摘要综合判断挑选；有新文章或列表为空时调用 API；显示最后更新时间；支持折叠让位给文章列表（与文章列表互斥联动，单字段状态 cat 切换自动 reset）
...
- **文章列表自适应** — 默认折叠；展开时自动折叠推荐区互让空间；展开后高度 min 120 / max 480px（推荐折叠后让出 ~400pt，文章列表可见 ~5 行）；popover 总高 maxHeight 1000 兜底防溢出
...
| 测试 | XCTest + Swift Testing（共 263 测试：254 XCTest + 9 Swift Testing）|
...
├── Services/
│   ├── BuiltInFeeds.swift       # 26 内置源（11 AI + 8 财报 + 7 新闻）含 cat
...
> 这是个人工具，使用 ad-hoc 签名（**未经 Apple 公证**）。首次打开时会被 macOS Gatekeeper 拦截（提示"无法验证"或"已损坏"），按下面步骤走一次即可，之后正常双击启动。
...
### 步骤 2 ：用终端解除隔离（推荐，macOS 15+ / 26 唯一稳定路径）

直接双击会弹"无法验证 / 已损坏"对话框，且 macOS 15 Sequoia 起**右键打开已被移除、系统设置「仍要打开」按钮对 ad-hoc 签名也常常不出现**。最可靠的办法是用终端清掉下载时附加的隔离标记：
...
3. 之后正常双击启动，菜单栏右上角会出现「资讯助手」图标

> 这条命令只是"告诉 macOS 这个文件不是从网上下载的"，**不修改任何文件内容**，是安全的。它是目前 ad-hoc 应用在新版 macOS 上最省事、成功率最高的路径。

### 步骤 2'（可选）：纯 GUI 尝试

如果实在不想碰终端，可以先试 GUI —— 但在 macOS 15+/26 上对 ad-hoc 应用**不保证有效**：

1. 双击「资讯助手」触发一次"无法打开"对话框（先失败一次，系统才会登记）
2. 立刻打开「**系统设置 → 隐私与安全性**」，滚到底部「**安全性**」区块
3. 若出现「"资讯助手" 已被阻止...」+「**仍要打开**」按钮，点它并输入密码
4. **若没有该按钮**（macOS 26 对 ad-hoc 常见），回到步骤 2 用终端
...
- `ModelsTests` / `BuiltInFeedsTests` — 模型默认值、26 内置源 cat 分布
...
### 新闻 tab（7，时政/社会/国际，去科技去娱乐）
NYT World · BBC World · FT 中文新闻 · 新华网 时政 · 人民日报 时政 · 新华网 社会 · 澎湃新闻

> 2026-05-29 聚焦实时/社会/国际新闻：删旧科技源（Hacker News / The Verge / 36 氪，科技交给 AI tab）；BBC 综合换分版块 **World**（去娱乐/体育）；补社会民生维度（新华网社会版官方直连 + 澎湃市场化视角）。**澎湃** 走公共 RSSHub 镜像 `rsshub.rssforever.com`（同财报源 known risk，挂了仅此源空白，其余 6 源官方直连不受影响）。
