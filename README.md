...
- **per-tab 后台刷新** — 1 小时 timer 并发刷三 cat（峰值 3×5=15 路对 DashScope 30 QPS 安全）；user 可通用 Tab 关掉某 cat 省 token
...
| 测试 | XCTest + Swift Testing（共 208 测试：199 XCTest + 9 Swift Testing）|
...
│   ├── RefreshService.swift     # 编排者；states (private set) + per-cat inflight + 并发 timer + markAvailability
...
**测试覆盖（208 case = 199 XCTest + 9 Swift Testing）**：
