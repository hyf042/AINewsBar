...
| `App/AppDelegate.swift` | **冷启动 entry**：`static let container` + `applicationDidFinishLaunching` 执行所有 startup（UsageRecorder + cleanup + configure + feed sync + postUnreadCount + launchBackgroundRefreshIfNeeded）；隐藏 Dock 图标 |

### Services（外观 + 组件）
| 文件 | 说明 |
|------|------|
| `Services/RefreshService.swift` | **外观** (~420 行)：聚合 @Published UI 状态 + 编排 RSS/Pipeline/Engine + 原子 `commit(Outcome)`；`refresh` / `forceRegenerateRecommend` / `forceRegenerateDigest` 三个公开入口；force/auto 路径通过 Trigger enum 合一；含 `resetCrossedDayStateIfNeeded()` 跨日全量重置（timer + refreshIfNeeded 双调用） |
